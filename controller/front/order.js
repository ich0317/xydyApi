...
exports.placeOrder = async (req, res, next) => {
  ...
  //1查找此用户是否有未完成订单
  const findOrder = await orderListTable.findOne(
    { user_id, status: ORDER_STATUS[0] },
    (err, data) => data
  );
  //2查找座位是否被占用
  let findSeat = await ticketsListTable.find(
    {
      session_id,
      $nor: [{ user_id }]
    },
    async (err, data) => {
      ...
      if (used == 0) {
        //座位可用
        //3查找影片信息
        let findFilmInfo = await sessionListTable.findOne(
          { _id: session_id },
          "film_name film_version start_datetime end_datetime screen_name cinema_id screen_id film_photo language sell_price",
          (err, data) => data
        );
        //4查找影院服务费
        const findServePrice = await cinemaListTable.findOne(
          { _id: findFilmInfo.cinema_id },
          "cinema_name serve_price",
          (err, data) => data
        );
        //座位id找相应座位号
        let seat = [];
        const findSetInfo = await seatListTable.find(
          { screen_id: findFilmInfo.screen_id, _id: seat_id },
          "seat_col seat_row seat_status"
        );
        ...
        let ticketInfo = findSetInfo.map(v => {
          return {
            seat_id: v._id,
            seat_status: SEAT_STATUS[2], //锁定
            user_id,
            session_id,
            seat: `${v.seat_row}排${v.seat_col}座`,
            ticket_code: null,
            refund_datetime: null,
            ticket_status: 0 // 0未支付 1已售 2已退
          };
        });

        if (findOrder) {
          //修改订单
          orderListTable.updateOne(
            { _id: findOrder._id },
            createInfo,
            async (err, data) => {
              res.json({
          ...
          ticketInfo.forEach(v => (v.order_id = findOrder._id));
          const delRes = await ticketsListTable.deleteMany({order_id:findOrder._id});
          if(delRes)ticketsListTable.insertMany(ticketInfo);
          ...
          orderListTable.create(createInfo, (err, data) => {
            ...
            ticketInfo.forEach(v => (v.order_id = data._id));
...
exports.cancelOrder = (req, res, next) => {
  ...
  orderListTable.deleteOne({ _id: order_id }, (err, data) => {
    ...
    ticketsListTable.deleteMany({ order_id }, (err, data) =>
      console.error(err)
    );
  });
};
...
exports.payOrder = (req, res, next) => {
  ...
  orderListTable.updateOne(
    { _id: order_id },
    {
      ...
      pay_datetime: stampToTime(Date.now(), "YMDhms"),
      QR: base64QR
    },
    (err, data) => {
      res.json({
      ...
      ticketsListTable.updateMany(
        { order_id },
        { seat_status: SEAT_STATUS[1], ticket_code: "123", ticket_status: 1 },
...
let timer = setInterval(() => {
  orderListTable.find({ status: ORDER_STATUS[0] }, (err, data) => {
    ...
    let expireOrder = data.map(v => {
      let placeOrderTime = parseInt(timeToStamp(v.order_datetime) / 1000) + 300;
    ...
    if (expireOrder.length != 0) {
      orderListTable.deleteMany({ _id: expireOrder }, (err, data) => {
        if (data) {
          ...
          ticketsListTable.deleteMany({ order_id: delCon }, (err, data) =>
            err
          );
        }
      });
...
exports.getOrderList = (req, res, err) => {
  ...
  orderListTable
    .find(
      { user_id },
      "cinema_name film_name start_datetime seat status end_datetime"
    )
    .sort({ _id: -1 })
    .exec((err, data) => {
      if (data.length == 0) {
